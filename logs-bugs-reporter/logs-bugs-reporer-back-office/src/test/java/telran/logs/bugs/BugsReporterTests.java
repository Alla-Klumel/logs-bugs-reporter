...
public EmailBugCountTest() {
	...
	List<BugResponseDto> expectedBugsAfterColose = Arrays.asList(expectedAssigned1, expectedClosed2, expectedAssigned3);
	List<SeriousnessBugCount> seriousnessBugDistribution = Arrays.asList
...
WebTestClient testClient;
	...
	void closeBug2() {
		...
		testClient.put().uri(BUGS_CLOSE).contentType(MediaType.APPLICATION_JSON).bodyValue(closeData).exchange().expectStatus().isOk();
	}
	...
	void seriousnessDistribution() {
		...
		.expectBodyList(SeriousnessBugCount.class).isEqualTo(seriousnessBugDistribution);
	}
	@Test
	...
	void invalidOpenAndAssign() {
		...
		testClient.post().uri(BUGS_OPEN_ASSIGN).contentType(MediaType.APPLICATION_JSON).bodyValue(invalidBugAssignDto).exchange().expectStatus().isNotFound();
	}

	public void invalidGetRequest(String uriStr) {
