	...
	void takeAndSaveLogDto(LogDto logDto) {
		...
		LOG.debug("received log:{}", logDto);		
		 ...
		 if (!violations.isEmpty()) {
			violations.forEach(cv -> LOG.error("logDto:{};field{}; message:{}", 
					logDto, cv.getPropertyPath(), cv.getMessage()));
			LogDto exceptionLog = new LogDto(new Date(), LogType.BAD_REQUEST_EXCEPTION, LogsDbPopulatorAppl.class.getName(), 0,
					violations.toString());
